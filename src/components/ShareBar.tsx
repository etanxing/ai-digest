...
function Pill({
  ...
  const cls =
    "inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm cursor-pointer transition-opacity hover:opacity-70 select-none";
  const style = { border: "1px solid var(--border)", color: "var(--fg)", background: "var(--bg)" };
...
export default function ShareBar({ url, title }: { url: string; title: string }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      {/* Native share — only shown when available (mobile) */}
      {canNativeShare && (
        <Pill onClick={nativeShare}>
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
